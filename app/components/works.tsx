...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onHover }) => {
  ...
  // Shared button classes to ensure consistent sizing across variants
  const baseButtonClasses = "inline-flex items-center justify-center gap-2 text-xs h-10 px-4";
  ...
  const renderActionButtons = () => {
    ...
    if (project.figmaUrl) {
      buttons.push(
        ...
        >
          <button className={`btn-ghost ${baseButtonClasses}`}>
    ...
    if (project.githubUrl) {
      buttons.push(
        ...
        >
          <button className={`btn-secondary ${baseButtonClasses}`}>
    ...
    if (project.liveUrl) {
      buttons.push(
        ...
        >
          <button className={`btn-primary ${baseButtonClasses}`}>
      ...
      <div className="flex justify-center items-center h-64 mb-6">
        <div className="relative w-full h-full">
          ...
          {['alma-villa', 'b-dazzle-cafe'].includes(project.id) ? (
            ...
            <div className="relative overflow-hidden rounded-xl w-full h-full flex items-center justify-center bg-neutral-100 dark:bg-neutral-800/30">
              <Image
                src={project.image}
                alt={project.title}
                width={400}
